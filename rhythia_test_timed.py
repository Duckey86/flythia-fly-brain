...
timing_label = tk.Label(
    root,
    text="Decision: -- ms   Motor: -- ms   Total: -- ms",
    font=("Arial", 11)
)
timing_label.pack()
...
def choose_action():
    ...
    # Measure ONLY the live GPU neural decision.
...
def ai_step():
    ...
    print(
        f"GPU decision: {decision_ms:.1f} ms"
    )
...
def run_motor_action(
    ...
    try:
        ...
        # Measure ONLY the actual motor-circuit simulation.
        ...
        total_ms = (
            decision_ms + motor_ms
        )

        print(
            f"Motor circuit: {motor_ms:.1f} ms"
        )

        print(
            f"TOTAL MOVE: {total_ms:.1f} ms"
        )
